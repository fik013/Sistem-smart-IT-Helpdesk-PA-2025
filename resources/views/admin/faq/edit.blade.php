...
<div class="flex flex-col gap-6 p-4 md:p-8 max-w-[800px] mx-auto w-full">
    ...
    <div class="flex flex-wrap gap-2 text-sm">
        ...
        <span class="text-slate-400 dark:text-[#92a9c9] font-medium">/</span>
        <span class="text-slate-800 dark:text-white font-medium">Edit FAQ</span>
    </div>

    <!-- Page Heading -->
    <div class="flex flex-col gap-2">
        <h1 class="text-slate-900 dark:text-white text-3xl font-black leading-tight tracking-[-0.033em]">Edit FAQ</h1>
        <p class="text-slate-500 dark:text-[#92a9c9] text-base font-normal">Update existing question and answer.</p>
    </div>
        ...
        <form action="{{ route('admin.faq.update', $faq) }}" method="POST" class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea name="answer" id="answer" rows="6" 
                          class="w-full rounded-lg border border-slate-300 dark:border-[#324867] bg-white dark:bg-[#111822] p-2.5 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-[#92a9c9] focus:border-primary focus:ring-primary focus:outline-none transition-all @error('answer') border-red-500 @enderror">{{ old('answer', $faq->answer) }}</textarea>
                @error('answer')
                    <p class="text-red-400 text-xs">{{ $message }}</p>
                @enderror
            </div>
