...
<div class="flex flex-col gap-6 p-4 md:p-8 max-w-[800px] mx-auto w-full">
    ...
    <div class="flex flex-wrap gap-2 text-sm">
        ...
        <span class="text-slate-400 dark:text-[#92a9c9] font-medium">/</span>
        <span class="text-slate-800 dark:text-white font-medium">Tambah Aset</span>
    </div>
    ...
    <div class="rounded-xl border border-slate-200 dark:border-[#233348] bg-white dark:bg-[#1a232e] p-6">
        <form action="{{ route('admin.inventory.store') }}" method="POST" class="flex flex-col gap-6" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="owner_type" value="user" class="w-4 h-4 text-primary bg-white dark:bg-[#111822] border-slate-300 dark:border-slate-500 focus:ring-primary focus:ring-2" {{ old('owner_type', 'user') == 'user' ? 'checked' : '' }} onchange="toggleOwnerType()">
                        <span class="text-slate-700 dark:text-white text-sm">Pegawai (Perorangan)</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="owner_type" value="department" class="w-4 h-4 text-primary bg-white dark:bg-[#111822] border-slate-300 dark:border-slate-500 focus:ring-primary focus:ring-2" {{ old('owner_type') == 'department' ? 'checked' : '' }} onchange="toggleOwnerType()">
            ...
            <!-- Image Upload -->
             <div class="flex flex-col gap-2">
                <label for="image" class="text-slate-700 dark:text-white text-sm font-medium">Foto Aset (Opsional)</label>
                ...
                <p class="text-xs text-slate-500 dark:text-text-secondary">Format: JPG, PNG, JPEG. Max: 2MB.</p>
                @error('image')
                    <p class="text-red-400 text-xs">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea name="description" id="description" rows="4" 
                          class="w-full rounded-lg border border-slate-300 dark:border-[#324867] bg-white dark:bg-[#111822] p-2.5 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-[#92a9c9] focus:border-primary focus:ring-primary focus:outline-none transition-all @error('description') border-red-500 @enderror"
