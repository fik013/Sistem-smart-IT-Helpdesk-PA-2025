...
<div class="flex flex-col gap-6 p-4 md:p-8 max-w-[1200px] mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div class="flex flex-col gap-2">
            <h1 class="text-[#101822] dark:text-white text-3xl md:text-4xl font-black leading-tight tracking-[-0.033em]">Manajemen Aset</h1>
            <p class="text-slate-500 dark:text-[#92a9c9] text-base font-normal">Kelola aset hardware dan software yang dimiliki oleh pengguna.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div class="flex flex-col gap-2 rounded-xl p-5 border border-slate-200 dark:border-[#233348] bg-white dark:bg-[#1a232e]">
            <div class="flex justify-between items-start">
                <p class="text-slate-500 dark:text-[#92a9c9] text-sm font-medium uppercase tracking-wider">Total Aset</p>
        ...
        <div class="flex flex-col gap-2 rounded-xl p-5 border border-slate-200 dark:border-[#233348] bg-white dark:bg-[#1a232e]">
            <div class="flex justify-between items-start">
                <p class="text-emerald-600 dark:text-emerald-500 text-sm font-medium uppercase tracking-wider">Aktif</p>
        ...
        <div class="flex flex-col gap-2 rounded-xl p-5 border border-slate-200 dark:border-[#233348] bg-white dark:bg-[#1a232e]">
            <div class="flex justify-between items-start">
                <p class="text-yellow-600 dark:text-yellow-500 text-sm font-medium uppercase tracking-wider">Perbaikan</p>
        ...
        <div class="flex flex-col gap-2 rounded-xl p-5 border border-slate-200 dark:border-[#233348] bg-white dark:bg-[#1a232e]">
            <div class="flex justify-between items-start">
                <p class="text-red-600 dark:text-red-500 text-sm font-medium uppercase tracking-wider">Rusak/Afkir</p>
            ...
            <table class="w-full text-left text-sm text-slate-500 dark:text-[#92a9c9]">
                <thead class="bg-slate-100 dark:bg-[#233348] text-xs uppercase text-[#101822] dark:text-white font-semibold">
                    <tr>
                        ...
                        <th class="px-6 py-4" scope="col">Detail Item</th>
                        <th class="px-6 py-4" scope="col">Kategori</th>
                        <th class="px-6 py-4" scope="col">Dimiliki Oleh</th>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-[#233348]">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-[#233348]/50 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @else
                                <span class="text-slate-400 dark:text-[#92a9c9] italic">Belum Ditentukan</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium border
                                ...
                                {{ ucfirst($item->status === 'maintenance' ? 'Perbaikan' : ($item->status === 'retired' ? 'Rusak/Afkir' : ($item->status === 'active' ? 'Aktif' : $item->status))) }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-4 text-center">Tidak ada aset yang ditemukan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
