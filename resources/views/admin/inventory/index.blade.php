...
<div class="flex flex-col gap-6 p-4 md:p-8 max-w-[1200px] mx-auto">
    ...
    <div class="flex flex-wrap gap-2 text-sm">
        ...
        <span class="text-white font-medium">Kelola Inventaris</span>
    </div>

    <!-- Page Heading & Actions -->
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div class="flex flex-col gap-2">
            <h1 class="text-white text-3xl md:text-4xl font-black leading-tight tracking-[-0.033em]">Asset Management</h1>
            <p class="text-[#92a9c9] text-base font-normal">Manage hardware and software assets assigned to users.</p>
        </div>
                    ...
                    <tr class="hover:bg-[#233348]/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span class="font-medium text-white">{{ $item->item_name }}</span>
                                <span class="text-xs">S/N: {{ $item->serial_number ?? 'N/A' }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <div class="h-6 w-6 rounded-full bg-cover bg-center" style='background-image: url("https://ui-avatars.com/api/?name={{ urlencode($item->user->name ?? 'Unassigned') }}&size=24");'></div>
                                <span class="text-white">{{ $item->user->name ?? 'Unassigned' }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium border
                                {{ $item->status === 'active' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 
                                   ($item->status === 'maintenance' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20') }}">
                                {{ ucfirst($item->status) }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('admin.inventory.edit', $item) }}" class="rounded-lg p-2 text-[#92a9c9] hover:bg-primary/20 hover:text-white transition-colors" title="Edit Item">
                                ...
                                <form action="{{ route('admin.inventory.destroy', $item) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="rounded-lg p-2 text-[#92a9c9] hover:bg-red-500/20 hover:text-red-400 transition-colors" title="Delete Item">
